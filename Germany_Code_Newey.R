...
#Import Germany Dataset
Germany_Data_Date <- read.csv("~/Desktop/MA/Data/Germany_Data.csv") 

Germany_Data_Date$fic <- NULL

# Include only common stocks
Germany_Data_Date <- Germany_Data_Date[which(Germany_Data_Date$tpci == 0),]

# Convert DM to EUR
EUR_DM_Conversion <- Germany_Data_Date[which(Germany_Data_Date$curcdd == "DEM"),]
Germany_Data_Date <- Germany_Data_Date[which(Germany_Data_Date$curcdd == "EUR"),]

EUR_DM_Conversion$prccd <- EUR_DM_Conversion$prccd / 1.95583
EUR_DM_Conversion$div <- EUR_DM_Conversion$div / 1.95583

Germany_Data_Date <- rbind(Germany_Data_Date, EUR_DM_Conversion)

rm(EUR_DM_Conversion)

Germany_Data_Date$curcdd <- NULL
Germany_Data_Date$tpci <- NULL

### Add Inflation column to dataset
Germany_Inflation <- select(read_excel("Desktop/MA/Data/Inflation-data.xlsx"), date, Germany)# Headline CPI; Data from Worldbank: https://www.worldbank.org/en/research/brief/inflation-database ; (Monthly inflation following this methodology: https://www.uvm.edu/~awoolf/classes/spring2005/ec11/calculating_inflation.html)
Germany_Inflation <-  Germany_Inflation %>%mutate(Germany=lag(Germany)) %>%na.omit()

# Remove ADR´s 
Germany_Data_Date$ADR <- substr(Germany_Data_Date$iid, 1,2)
Germany_Data_Date$ADR <- as.numeric(as.character(Germany_Data_Date$ADR))
Germany_Data_Date <- Germany_Data_Date[!Germany_Data_Date$ADR >= 90,]
Germany_Data_Date$ADR <- NULL 

#Create separate identification column
Identification <- Germany_Data_Date[c("gvkey","ggroup", "gind", "naics", "sic","conm")]
...
Germany_Data_Date$ggroup  <- NULL
Germany_Data_Date$gind  <- NULL
Germany_Data_Date$naics  <- NULL
Germany_Data_Date$sic  <- NULL

#Exclude rows which only have the dividend information of the secondary issue (div amount of primary issue is copied)

Germany_Data_Date <- Germany_Data_Date[!is.na(Germany_Data_Date$prccd),]

#create merged column to address the problem of multiple issues
Germany_Data_Date$merged <- paste(Germany_Data_Date$gvkey, Germany_Data_Date$iid)

# Replace missing trfd values with last available value
Germany_Data_Date <- as.data.frame(Germany_Data_Date %>% group_by(merged) %>% fill(trfd) %>% fill(trfd))
Germany_Data_Date$trfd[is.na(Germany_Data_Date$trfd)] <- 1

Germany_Data_Date$iid <- NULL
Germany_Data_Date$mktcap <-abs(Germany_Data_Date$prccd)*Germany_Data_Date$cshoc

dummy <- aggregate(mktcap ~ datadate + gvkey, Germany_Data_Date, na.action=na.pass, sum)  
names(Germany_Data_Date)[names(Germany_Data_Date) == 'mktcap'] <- 'indiv_merged_mktcap'

Germany_Data_Date <- join(dummy,Germany_Data_Date,by=c("datadate", "gvkey"), type="inner")

Germany_Data_Date$frct <- Germany_Data_Date$indiv_merged_mktcap / Germany_Data_Date$mktcap
dummy<- Germany_Data_Date %>% group_by(gvkey, datadate) %>% summarise(n = n())

Germany_Data_Date <- join(Germany_Data_Date,dummy,by=c("datadate", "gvkey"), type="inner")

dummy <- Germany_Data_Date[is.na(Germany_Data_Date$frct),]
dummy$frct[is.na(dummy$frct)] <- 1/dummy$n

Germany_Data_Date <- Germany_Data_Date[!is.na(Germany_Data_Date$frct),]
Germany_Data_Date <- rbind(Germany_Data_Date,dummy)

Germany_Data_Date$indiv_merged_mktcap <- NULL
Germany_Data_Date$n <- NULL

Germany_Data_Date$PrAj_1 <- Germany_Data_Date$prccd / Germany_Data_Date$ajexdi
Germany_Data_Date$PrAjTR_1 <- Germany_Data_Date$PrAj_1 * Germany_Data_Date$trfd

library(data.table)

Germany_Data_Date <- as.data.table(Germany_Data_Date)
setkey(Germany_Data_Date, gvkey, datadate)

#Subset the div column
dividend <- Germany_Data_Date[,c("datadate", "gvkey", "div", "prccd", "frct", "merged", "ajexdi")]

Germany_Data_Date <- Germany_Data_Date[!is.infinite(Germany_Data_Date$PrAjTR_1),]
Germany_Data_Date[, return := PrAjTR_1 / shift(PrAjTR_1, fill = NA), by = merged]
Germany_Data_Date$return[is.na(Germany_Data_Date$return)] <- 1

# Remove observations with daily returns >20
Germany_Data_Date <- Germany_Data_Date[which(Germany_Data_Date$return <= 20),]
Germany_Data_Date <- Germany_Data_Date[which(Germany_Data_Date$return >= 0.05),]

Germany_Data_Date$frct_return <- Germany_Data_Date$return * Germany_Data_Date$frct
Germany_Data_Date$return <- NULL
dummy <- aggregate(frct_return ~ datadate + gvkey, Germany_Data_Date, na.action=na.pass,sum)
names(dummy)[names(dummy) == 'frct_return'] <- 'return'

Germany_Data_Date <- join(Germany_Data_Date, dummy, by=c("datadate","gvkey"), type="inner")

Germany_Data_Date <- Germany_Data_Date[!duplicated(Germany_Data_Date[,c("datadate","gvkey")]),]

Germany_Data_Date$Month_Year <- substr(Germany_Data_Date$datadate, 1,6)

#subset volatility dataset
volatility <- select(Germany_Data_Date, datadate, gvkey, return)
names(volatility)[names(volatility) == 'return_dummy'] <- 'return'


Germany_Data_Date$merged <- NULL
Germany_Data_Date$frct <- NULL
Germany_Data_Date$frct_return <- NULL

Germany_Data_Date$PrAj_1  <- NULL
Germany_Data_Date$PrAjTR_1 <- NULL

Germany_Data_Date$ajexdi <- NULL
Germany_Data_Date$trfd <- NULL
Germany_Data_Date$div <- NULL
...
library(data.table)
dummy <- setDT(Germany_Data_Date)[,prod(return), by=.(Month_Year,gvkey)]

#merge the datasets 
Germany <- join(dummy, Germany_Data_Date, by=c("Month_Year","gvkey"), type="inner")
rm(dummy)

#rename monthly return column in ret
names(Germany)[names(Germany) == 'V1'] <- 'ret'

# Remove the duplicates
Germany <- Germany[!duplicated(Germany[,c("Month_Year","gvkey")])]

#Remove excess columns
Germany$return <- NULL
Germany$datadate <- NULL

# Transform returns into 0.2 = 20% notation as in original Germany dataset
Germany$ret <- Germany$ret -1

# Rename columns to be equivalent to Original Germany dataset
names(Germany)[names(Germany) == 'Month_Year'] <- 'date'
names(Germany)[names(Germany) == 'gvkey'] <- 'PERMNO'
names(Germany)[names(Germany) == 'ret'] <- 'RET'
names(Germany)[names(Germany) == 'conm'] <- 'COMNAM'
names(Germany)[names(Germany) == 'cshoc'] <- 'SHROUT'
names(Germany)[names(Germany) == 'prccd'] <- 'PRC'


#Rename inflation dataset accordingly

names(Germany_Inflation)[names(Germany_Inflation) == 'Germany'] <- 'inflation'

# Add Inflation Data
Germany$date <- as.numeric(as.character(Germany$date))
Original1 <- join(Germany,Germany_Inflation,by=c("date"), type="inner")
...
rm(Peter, Peter1, Original1, Germany)
...
Germany_Shares_Outstanding <- select(read.csv("~/Desktop/MA/Data/full_shares_set_global.csv") , gvkey, datadate, cshoi)  # Retreived from quarterly fundamentals compGermanytat
Germany_Shares_Outstanding <- transform(Germany_Shares_Outstanding, datadate = as.Date(as.character(datadate), "%Y%m%d"))
Germany_Shares_Outstanding <- as.data.table(Germany_Shares_Outstanding)
library(lubridate)
Germany_Shares_Outstanding <- Germany_Shares_Outstanding[rep(1:.N, each = 12),]
Germany_Shares_Outstanding[, year := datadate]
Germany_Shares_Outstanding[, datadate := {day(datadate) <- 1; datadate}]
Germany_Shares_Outstanding[, datadate := {month(datadate) <- month(datadate) - 0:11; datadate}, by = year]
Germany_Shares_Outstanding$datadate <- gsub("-", "", Germany_Shares_Outstanding$datadate)
Germany_Shares_Outstanding$year <- NULL
Germany_Shares_Outstanding$cshoi <- Germany_Shares_Outstanding$cshoi *1000000
Germany_Shares_Outstanding$datadate <- substr(Germany_Shares_Outstanding$datadate, 1,6)

names(Germany_Shares_Outstanding)[names(Germany_Shares_Outstanding) == 'gvkey'] <- 'PERMNO'
names(Germany_Shares_Outstanding)[names(Germany_Shares_Outstanding) == 'datadate'] <- 'date'

dummy <- InflaData[is.na(InflaData$mktcap),]
dummy <- join(Germany_Shares_Outstanding, dummy, by=c("date","PERMNO"), type="right")
...
FF5 <- read_excel("~/Desktop/MA/Data/FF5_Germany.xlsx")
...
Germany_Inflation$date <- as.numeric(as.character(Germany_Inflation$date))


# Add the inflation column
Rebalanced_Data <- merge(Rebalanced_Data,Germany_Inflation,by=c("date"))
...
dividend_Q1_Q5$tot_port_ret <- dividend_Q1_Q5$tot_port_ret - dividend_Q5$tot_port_ret
