...
#Import France Dataset
France_Data_Date <- read.csv("~/Desktop/MA/Data/France_Data.csv") 

France_Data_Date$fic <- NULL
# Include only common stocks
France_Data_Date <- France_Data_Date[which(France_Data_Date$tpci == 0),]

# Convert DM to EUR
EUR_FRF_Conversion <- France_Data_Date[which(France_Data_Date$curcdd == "FRF"),]
France_Data_Date <- France_Data_Date[which(France_Data_Date$curcdd == "EUR"),]

EUR_FRF_Conversion$prccd <- EUR_FRF_Conversion$prccd / 6.55957
EUR_FRF_Conversion$div <- EUR_FRF_Conversion$div / 6.55957

France_Data_Date <- rbind(France_Data_Date, EUR_FRF_Conversion)

rm(EUR_FRF_Conversion)

France_Data_Date$curcdd <- NULL
France_Data_Date$tpci <- NULL

### Add Inflation column to dataset
France_Inflation <- select(read_excel("Desktop/MA/Data/Inflation-data.xlsx"), date, France)# Headline CPI; Data from Worldbank: https://www.worldbank.org/en/research/brief/inflation-database ; (Monthly inflation following this methodology: https://www.uvm.edu/~awoolf/classes/spring2005/ec11/calculating_inflation.html)
France_Inflation <-  France_Inflation %>%mutate(France=lag(France)) %>%na.omit()

# Remove ADR´s 
France_Data_Date$ADR <- substr(France_Data_Date$iid, 1,2)
France_Data_Date$ADR <- as.numeric(as.character(France_Data_Date$ADR))
France_Data_Date <- France_Data_Date[!France_Data_Date$ADR >= 90,]
France_Data_Date$ADR <- NULL 

#Create separate identification column
Identification <- France_Data_Date[c("gvkey","ggroup", "gind", "naics", "sic", "conm")]
...
France_Data_Date$ggroup  <- NULL
France_Data_Date$gind  <- NULL
France_Data_Date$naics  <- NULL
France_Data_Date$sic  <- NULL

#Exclude rows which only have the dividend information of the secondary issue (div amount of primary issue is copied)

France_Data_Date <- France_Data_Date[!is.na(France_Data_Date$prccd),]

#create merged column to address the problem of multiple issues
France_Data_Date$merged <- paste(France_Data_Date$gvkey, France_Data_Date$iid)

# Replace missing trfd values with last available value
France_Data_Date <- as.data.frame(France_Data_Date %>% group_by(merged) %>% fill(trfd) %>% fill(trfd))
France_Data_Date$trfd[is.na(France_Data_Date$trfd)] <- 1

France_Data_Date$iid <- NULL
France_Data_Date$mktcap <-abs(France_Data_Date$prccd)*France_Data_Date$cshoc

dummy <- aggregate(mktcap ~ datadate + gvkey, France_Data_Date, na.action=na.pass, sum)  
names(France_Data_Date)[names(France_Data_Date) == 'mktcap'] <- 'indiv_merged_mktcap'

France_Data_Date <- join(dummy,France_Data_Date,by=c("datadate", "gvkey"), type="inner")

France_Data_Date$frct <- France_Data_Date$indiv_merged_mktcap / France_Data_Date$mktcap
dummy<- France_Data_Date %>% group_by(gvkey, datadate) %>% summarise(n = n())

France_Data_Date <- join(France_Data_Date,dummy,by=c("datadate", "gvkey"), type="inner")

dummy <- France_Data_Date[is.na(France_Data_Date$frct),]
dummy$frct[is.na(dummy$frct)] <- 1/dummy$n

France_Data_Date <- France_Data_Date[!is.na(France_Data_Date$frct),]
France_Data_Date <- rbind(France_Data_Date,dummy)

France_Data_Date$indiv_merged_mktcap <- NULL
France_Data_Date$n <- NULL

France_Data_Date$PrAj_1 <- France_Data_Date$prccd / France_Data_Date$ajexdi
France_Data_Date$PrAjTR_1 <- France_Data_Date$PrAj_1 * France_Data_Date$trfd

library(data.table)

France_Data_Date <- as.data.table(France_Data_Date)
setkey(France_Data_Date, gvkey, datadate)

#Subset the div column
dividend <- France_Data_Date[,c("datadate", "gvkey", "div", "prccd", "frct", "merged", "ajexdi")]

France_Data_Date <- France_Data_Date[!is.infinite(France_Data_Date$PrAjTR_1),]
France_Data_Date[, return := PrAjTR_1 / shift(PrAjTR_1, fill = NA), by = merged]
France_Data_Date$return[is.na(France_Data_Date$return)] <- 1

# Remove observations with daily returns >20
France_Data_Date <- France_Data_Date[which(France_Data_Date$return <= 20),]
France_Data_Date <- France_Data_Date[which(France_Data_Date$return >= 0.05),]

France_Data_Date$frct_return <- France_Data_Date$return * France_Data_Date$frct
France_Data_Date$return <- NULL
dummy <- aggregate(frct_return ~ datadate + gvkey, France_Data_Date, na.action=na.pass,sum)
names(dummy)[names(dummy) == 'frct_return'] <- 'return'

France_Data_Date <- join(France_Data_Date, dummy, by=c("datadate","gvkey"), type="inner")

France_Data_Date <- France_Data_Date[!duplicated(France_Data_Date[,c("datadate","gvkey")]),]

France_Data_Date$Month_Year <- substr(France_Data_Date$datadate, 1,6)

#subset volatility dataset
volatility <- select(France_Data_Date, datadate, gvkey, return)
names(volatility)[names(volatility) == 'return_dummy'] <- 'return'


France_Data_Date$merged <- NULL
France_Data_Date$frct <- NULL
France_Data_Date$frct_return <- NULL

France_Data_Date$PrAj_1  <- NULL
France_Data_Date$PrAjTR_1 <- NULL

France_Data_Date$ajexdi <- NULL
France_Data_Date$trfd <- NULL
France_Data_Date$div <- NULL
...
library(data.table)
dummy <- setDT(France_Data_Date)[,prod(return), by=.(Month_Year,gvkey)]

#merge the datasets 
France <- join(dummy, France_Data_Date, by=c("Month_Year","gvkey"), type="inner")
rm(dummy)

#rename monthly return column in ret
names(France)[names(France) == 'V1'] <- 'ret'

# Remove the duplicates
France <- France[!duplicated(France[,c("Month_Year","gvkey")])]

#Remove excess columns
France$return <- NULL
France$datadate <- NULL

# Transform returns into 0.2 = 20% notation as in original France dataset
France$ret <- France$ret -1

# Rename columns to be equivalent to Original France dataset
names(France)[names(France) == 'Month_Year'] <- 'date'
names(France)[names(France) == 'gvkey'] <- 'PERMNO'
names(France)[names(France) == 'ret'] <- 'RET'
names(France)[names(France) == 'conm'] <- 'COMNAM'
names(France)[names(France) == 'cshoc'] <- 'SHROUT'
names(France)[names(France) == 'prccd'] <- 'PRC'


#Rename inflation dataset accordingly

names(France_Inflation)[names(France_Inflation) == 'France'] <- 'inflation'

# Add Inflation Data
France$date <- as.numeric(as.character(France$date))
Original1 <- join(France,France_Inflation,by=c("date"), type="inner")
...
rm(Peter, Peter1, Original1, France)
...
France_Shares_Outstanding <- select(read.csv("~/Desktop/MA/Data/full_shares_set_global.csv") , gvkey, datadate, cshoi)  # Retreived from quarterly fundamentals compFrancetat
France_Shares_Outstanding <- transform(France_Shares_Outstanding, datadate = as.Date(as.character(datadate), "%Y%m%d"))
France_Shares_Outstanding <- as.data.table(France_Shares_Outstanding)
library(lubridate)
France_Shares_Outstanding <- France_Shares_Outstanding[rep(1:.N, each = 12),]
France_Shares_Outstanding[, year := datadate]
France_Shares_Outstanding[, datadate := {day(datadate) <- 1; datadate}]
France_Shares_Outstanding[, datadate := {month(datadate) <- month(datadate) - 0:11; datadate}, by = year]
France_Shares_Outstanding$datadate <- gsub("-", "", France_Shares_Outstanding$datadate)
France_Shares_Outstanding$year <- NULL
France_Shares_Outstanding$cshoi <- France_Shares_Outstanding$cshoi *1000000
France_Shares_Outstanding$datadate <- substr(France_Shares_Outstanding$datadate, 1,6)

names(France_Shares_Outstanding)[names(France_Shares_Outstanding) == 'gvkey'] <- 'PERMNO'
names(France_Shares_Outstanding)[names(France_Shares_Outstanding) == 'datadate'] <- 'date'

dummy <- InflaData[is.na(InflaData$mktcap),]
dummy <- join(France_Shares_Outstanding, dummy, by=c("date","PERMNO"), type="right")
...
FF5 <- read_excel("~/Desktop/MA/Data/FF5_France.xlsx")
...
France_Inflation$date <- as.numeric(as.character(France_Inflation$date))


# Add the inflation column
Rebalanced_Data <- merge(Rebalanced_Data,France_Inflation,by=c("date"))
...
library(sandwich)
library(lmtest)
coeftest(reg, NeweyWest(reg, lag = 20, prewhite = FALSE))
