...
#Import Canada Dataset
Canada_Data_Date <- read.csv("~/Desktop/MA/Data/Canada_Data.csv") 

# Include only stocks in domestic currency
Canada_Data_Date <- Canada_Data_Date[which(Canada_Data_Date$curcdd == "CAD"),]

# Include only common stocks
Canada_Data_Date <- Canada_Data_Date[which(Canada_Data_Date$tpci == 0),]

Canada_Data_Date$curcdd <- NULL
Canada_Data_Date$tpci <- NULL

### Add Inflation column to dataset
Canada_Inflation <- select(read_excel("Desktop/MA/Data/Inflation-data.xlsx"), date, Canada)# Headline CPI; Data from Worldbank: https://www.worldbank.org/en/research/brief/inflation-database ; (Monthly inflation following this methodology: https://www.uvm.edu/~awoolf/classes/spring2005/ec11/calculating_inflation.html)
Canada_Inflation <-  Canada_Inflation %>%mutate(Canada=lag(Canada)) %>%na.omit()

# Remove ADR´s 
Canada_Data_Date$ADR <- substr(Canada_Data_Date$iid, 1,2)
Canada_Data_Date$ADR <- as.numeric(as.character(Canada_Data_Date$ADR))
Canada_Data_Date <- Canada_Data_Date[!Canada_Data_Date$ADR >= 90,]
Canada_Data_Date$ADR <- NULL 

#Create separate identification column
Identification <- Canada_Data_Date[c("gvkey","ggroup", "gind", "naics", "sic","conm")]
...
Canada_Data_Date$ggroup  <- NULL
Canada_Data_Date$gind  <- NULL
Canada_Data_Date$naics  <- NULL
Canada_Data_Date$sic  <- NULL

#Exclude rows which only have the dividend information of the secondary issue (div amount of primary issue is copied)

Canada_Data_Date <- Canada_Data_Date[!is.na(Canada_Data_Date$prccd),]

#create merged column to address the problem of multiple issues
Canada_Data_Date$merged <- paste(Canada_Data_Date$gvkey, Canada_Data_Date$iid)

# Replace missing trfd values with last available value
Canada_Data_Date <- as.data.frame(Canada_Data_Date %>% group_by(merged) %>% fill(trfd) %>% fill(trfd))
Canada_Data_Date$trfd[is.na(Canada_Data_Date$trfd)] <- 1

Canada_Data_Date$iid <- NULL
Canada_Data_Date$mktcap <-abs(Canada_Data_Date$prccd)*Canada_Data_Date$cshoc

dummy <- aggregate(mktcap ~ datadate + gvkey, Canada_Data_Date, na.action=na.pass, sum)  
names(Canada_Data_Date)[names(Canada_Data_Date) == 'mktcap'] <- 'indiv_merged_mktcap'

Canada_Data_Date <- join(dummy,Canada_Data_Date,by=c("datadate", "gvkey"), type="inner")

Canada_Data_Date$frct <- Canada_Data_Date$indiv_merged_mktcap / Canada_Data_Date$mktcap
dummy<- Canada_Data_Date %>% group_by(gvkey, datadate) %>% summarise(n = n())

Canada_Data_Date <- join(Canada_Data_Date,dummy,by=c("datadate", "gvkey"), type="inner")

dummy <- Canada_Data_Date[is.na(Canada_Data_Date$frct),]
dummy$frct[is.na(dummy$frct)] <- 1/dummy$n

Canada_Data_Date <- Canada_Data_Date[!is.na(Canada_Data_Date$frct),]
Canada_Data_Date <- rbind(Canada_Data_Date,dummy)

Canada_Data_Date$indiv_merged_mktcap <- NULL
Canada_Data_Date$n <- NULL

Canada_Data_Date$PrAj_1 <- Canada_Data_Date$prccd / Canada_Data_Date$ajexdi
Canada_Data_Date$PrAjTR_1 <- Canada_Data_Date$PrAj_1 * Canada_Data_Date$trfd

library(data.table)

Canada_Data_Date <- as.data.table(Canada_Data_Date)
setkey(Canada_Data_Date, gvkey, datadate)

#Subset the div column
dividend <- Canada_Data_Date[,c("datadate", "gvkey", "div", "prccd", "frct", "merged", "ajexdi")]

Canada_Data_Date <- Canada_Data_Date[!is.infinite(Canada_Data_Date$PrAjTR_1),]
Canada_Data_Date[, return := PrAjTR_1 / shift(PrAjTR_1, fill = NA), by = merged]
Canada_Data_Date$return[is.na(Canada_Data_Date$return)] <- 1

# Remove observations with daily returns >20
Canada_Data_Date <- Canada_Data_Date[which(Canada_Data_Date$return <= 20),]
Canada_Data_Date <- Canada_Data_Date[which(Canada_Data_Date$return >= 0.05),]

Canada_Data_Date$frct_return <- Canada_Data_Date$return * Canada_Data_Date$frct
Canada_Data_Date$return <- NULL
dummy <- aggregate(frct_return ~ datadate + gvkey, Canada_Data_Date, na.action=na.pass,sum)
names(dummy)[names(dummy) == 'frct_return'] <- 'return'

Canada_Data_Date <- join(Canada_Data_Date, dummy, by=c("datadate","gvkey"), type="inner")

Canada_Data_Date <- Canada_Data_Date[!duplicated(Canada_Data_Date[,c("datadate","gvkey")]),]

Canada_Data_Date$Month_Year <- substr(Canada_Data_Date$datadate, 1,6)

#subset volatility dataset
volatility <- select(Canada_Data_Date, datadate, gvkey, return)
names(volatility)[names(volatility) == 'return_dummy'] <- 'return'


Canada_Data_Date$merged <- NULL
Canada_Data_Date$frct <- NULL
Canada_Data_Date$frct_return <- NULL

Canada_Data_Date$PrAj_1  <- NULL
Canada_Data_Date$PrAjTR_1 <- NULL

Canada_Data_Date$ajexdi <- NULL
Canada_Data_Date$trfd <- NULL
Canada_Data_Date$div <- NULL
...
library(data.table)
dummy <- setDT(Canada_Data_Date)[,prod(return), by=.(Month_Year,gvkey)]

#merge the datasets 
Canada <- join(dummy, Canada_Data_Date, by=c("Month_Year","gvkey"), type="inner")
rm(dummy)

#rename monthly return column in ret
names(Canada)[names(Canada) == 'V1'] <- 'ret'

# Remove the duplicates
Canada <- Canada[!duplicated(Canada[,c("Month_Year","gvkey")])]

#Remove excess columns
Canada$return <- NULL
Canada$datadate <- NULL

# Transform returns into 0.2 = 20% notation as in original Canada dataset
Canada$ret <- Canada$ret -1

# Rename columns to be equivalent to Original Canada dataset
names(Canada)[names(Canada) == 'Month_Year'] <- 'date'
names(Canada)[names(Canada) == 'gvkey'] <- 'PERMNO'
names(Canada)[names(Canada) == 'ret'] <- 'RET'
names(Canada)[names(Canada) == 'conm'] <- 'COMNAM'
names(Canada)[names(Canada) == 'cshoc'] <- 'SHROUT'
names(Canada)[names(Canada) == 'prccd'] <- 'PRC'


#Rename inflation dataset accordingly

names(Canada_Inflation)[names(Canada_Inflation) == 'Canada'] <- 'inflation'

# Add Inflation Data
Canada$date <- as.numeric(as.character(Canada$date))
Original1 <- join(Canada,Canada_Inflation,by=c("date"), type="inner")
...
rm(Peter, Peter1, Original1, Canada)
...
Canada_Shares_Outstanding <- select(read.csv("~/Desktop/MA/Data/full_shares_set.csv") , gvkey, datadate, cshoq)  # Retreived from quarterly fundamentals compCanadatat
Canada_Shares_Outstanding <- transform(Canada_Shares_Outstanding, datadate = as.Date(as.character(datadate), "%Y%m%d"))
Canada_Shares_Outstanding <- as.data.table(Canada_Shares_Outstanding)
library(lubridate)
Canada_Shares_Outstanding <- Canada_Shares_Outstanding[rep(1:.N, each = 12),]
Canada_Shares_Outstanding[, year := datadate]
Canada_Shares_Outstanding[, datadate := {day(datadate) <- 1; datadate}]
Canada_Shares_Outstanding[, datadate := {month(datadate) <- month(datadate) - 0:11; datadate}, by = year]
Canada_Shares_Outstanding$datadate <- gsub("-", "", Canada_Shares_Outstanding$datadate)
Canada_Shares_Outstanding$year <- NULL
Canada_Shares_Outstanding$cshoq <- Canada_Shares_Outstanding$cshoq *1000000
Canada_Shares_Outstanding$datadate <- substr(Canada_Shares_Outstanding$datadate, 1,6)

names(Canada_Shares_Outstanding)[names(Canada_Shares_Outstanding) == 'gvkey'] <- 'PERMNO'
names(Canada_Shares_Outstanding)[names(Canada_Shares_Outstanding) == 'datadate'] <- 'date'

dummy <- InflaData[is.na(InflaData$mktcap),]
dummy <- join(Canada_Shares_Outstanding, dummy, by=c("date","PERMNO"), type="right")
dummy <- unique(setDT(dummy), by = c("PERMNO", "date"))
dummy$mktcap <- dummy$SHROUT *dummy$PRC

dummy2 <- dummy[is.na(dummy$mktcap),]
dummy2$mktcap <- dummy2$cshoq *dummy2$PRC
...
Q1_Q5_Returns <- merge(Q1_Q5_Returns, Q5_Returns, by="date")

Q1_Q5_Returns$tot_port_ret.x <- Q1_Q5_Returns$tot_port_ret.x - Q1_Q5_Returns$tot_port_ret.y
Q1_Q5_Returns$tot_port_ret.x <- Q1_Q5_Returns$tot_port_ret.x +1
colnames(Q1_Q5_Returns)[colnames(Q1_Q5_Returns) == "tot_port_ret.x"] <- "tot_port_ret"
colnames(Q1_Q5_Returns)[colnames(Q1_Q5_Returns) == "inflation.x"] <- "inflation"
Q1_Q5_Returns$real_tot_port_ret.x <- NULL
Q1_Q5_Returns$tot_port_ret.y <- NULL
Q1_Q5_Returns$inflation.y <- NULL
Q1_Q5_Returns$real_tot_port_ret.y <- NULL
...
FF5 <- read_excel("~/Desktop/MA/Data/FF5_Canada.xlsx")
...
Canada_Inflation$date <- as.numeric(as.character(Canada_Inflation$date))


# Add the inflation column
Rebalanced_Data <- merge(Rebalanced_Data,Canada_Inflation,by=c("date"))
...
library(sandwich)
library(lmtest)
coeftest(reg, NeweyWest(reg, lag = 17, prewhite = FALSE))
