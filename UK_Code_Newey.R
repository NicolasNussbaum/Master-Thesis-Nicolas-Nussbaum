...
#Import UK Dataset
UK_Data_Date <- read.csv("~/Desktop/MA/Data/UK_Data.csv") 

# Include only stocks in domestic currency
UK_Data_Date <- UK_Data_Date[which(UK_Data_Date$curcdd == "GBP"),]

UK_Data_Date$fic <- NULL


# Include only common stocks
UK_Data_Date <- UK_Data_Date[which(UK_Data_Date$tpci == 0),]

UK_Data_Date$curcdd <- NULL
UK_Data_Date$tpci <- NULL

### Add Inflation column to dataset
UK_Inflation <- select(read_excel("Desktop/MA/Data/Inflation-data.xlsx"), date, UK)# Headline CPI; Data from Worldbank: https://www.worldbank.org/en/research/brief/inflation-database ; (Monthly inflation following this methodology: https://www.uvm.edu/~awoolf/classes/spring2005/ec11/calculating_inflation.html)
UK_Inflation <-  UK_Inflation %>%mutate(UK=lag(UK)) %>%na.omit()

# Remove ADR´s 
UK_Data_Date$ADR <- substr(UK_Data_Date$iid, 1,2)
UK_Data_Date$ADR <- as.numeric(as.character(UK_Data_Date$ADR))
UK_Data_Date <- UK_Data_Date[!UK_Data_Date$ADR >= 90,]
UK_Data_Date$ADR <- NULL 

#Create separate identification column
Identification <- UK_Data_Date[c("gvkey","ggroup", "gind", "naics", "sic","conm")]
...
UK_Data_Date$ggroup  <- NULL
UK_Data_Date$gind  <- NULL
UK_Data_Date$naics  <- NULL
UK_Data_Date$sic  <- NULL

#Exclude rows which only have the dividend information of the secondary issue (div amount of primary issue is copied)

UK_Data_Date <- UK_Data_Date[!is.na(UK_Data_Date$prccd),]

#create merged column to address the problem of multiple issues
UK_Data_Date$merged <- paste(UK_Data_Date$gvkey, UK_Data_Date$iid)

# Replace missing trfd values with last available value
UK_Data_Date <- as.data.frame(UK_Data_Date %>% group_by(merged) %>% fill(trfd) %>% fill(trfd))
UK_Data_Date$trfd[is.na(UK_Data_Date$trfd)] <- 1

UK_Data_Date$iid <- NULL
UK_Data_Date$mktcap <-abs(UK_Data_Date$prccd)*UK_Data_Date$cshoc

dummy <- aggregate(mktcap ~ datadate + gvkey, UK_Data_Date, na.action=na.pass, sum)  
names(UK_Data_Date)[names(UK_Data_Date) == 'mktcap'] <- 'indiv_merged_mktcap'

UK_Data_Date <- join(dummy,UK_Data_Date,by=c("datadate", "gvkey"), type="inner")

UK_Data_Date$frct <- UK_Data_Date$indiv_merged_mktcap / UK_Data_Date$mktcap
dummy<- UK_Data_Date %>% group_by(gvkey, datadate) %>% summarise(n = n())

UK_Data_Date <- join(UK_Data_Date,dummy,by=c("datadate", "gvkey"), type="inner")

dummy <- UK_Data_Date[is.na(UK_Data_Date$frct),]
dummy$frct[is.na(dummy$frct)] <- 1/dummy$n

UK_Data_Date <- UK_Data_Date[!is.na(UK_Data_Date$frct),]
UK_Data_Date <- rbind(UK_Data_Date,dummy)

UK_Data_Date$indiv_merged_mktcap <- NULL
UK_Data_Date$n <- NULL

UK_Data_Date$PrAj_1 <- UK_Data_Date$prccd / UK_Data_Date$ajexdi
UK_Data_Date$PrAjTR_1 <- UK_Data_Date$PrAj_1 * UK_Data_Date$trfd

library(data.table)

UK_Data_Date <- as.data.table(UK_Data_Date)
setkey(UK_Data_Date, gvkey, datadate)

#Subset the div column
dividend <- UK_Data_Date[,c("datadate", "gvkey", "div", "prccd", "frct", "merged", "ajexdi")]

UK_Data_Date <- UK_Data_Date[!is.infinite(UK_Data_Date$PrAjTR_1),]
UK_Data_Date[, return := PrAjTR_1 / shift(PrAjTR_1, fill = NA), by = merged]
UK_Data_Date$return[is.na(UK_Data_Date$return)] <- 1

# Remove observations with daily returns >20
UK_Data_Date <- UK_Data_Date[which(UK_Data_Date$return <= 20),]
UK_Data_Date <- UK_Data_Date[which(UK_Data_Date$return >= 0.05),]

UK_Data_Date$frct_return <- UK_Data_Date$return * UK_Data_Date$frct
UK_Data_Date$return <- NULL
dummy <- aggregate(frct_return ~ datadate + gvkey, UK_Data_Date, na.action=na.pass,sum)
names(dummy)[names(dummy) == 'frct_return'] <- 'return'

UK_Data_Date <- join(UK_Data_Date, dummy, by=c("datadate","gvkey"), type="inner")

UK_Data_Date <- UK_Data_Date[!duplicated(UK_Data_Date[,c("datadate","gvkey")]),]

UK_Data_Date$Month_Year <- substr(UK_Data_Date$datadate, 1,6)

#subset volatility dataset
volatility <- select(UK_Data_Date, datadate, gvkey, return)
names(volatility)[names(volatility) == 'return_dummy'] <- 'return'


UK_Data_Date$merged <- NULL
UK_Data_Date$frct <- NULL
UK_Data_Date$frct_return <- NULL

UK_Data_Date$PrAj_1  <- NULL
UK_Data_Date$PrAjTR_1 <- NULL

UK_Data_Date$ajexdi <- NULL
UK_Data_Date$trfd <- NULL
UK_Data_Date$div <- NULL
...
library(data.table)
dummy <- setDT(UK_Data_Date)[,prod(return), by=.(Month_Year,gvkey)]

#merge the datasets 
UK <- join(dummy, UK_Data_Date, by=c("Month_Year","gvkey"), type="inner")
rm(dummy)

#rename monthly return column in ret
names(UK)[names(UK) == 'V1'] <- 'ret'

# Remove the duplicates
UK <- UK[!duplicated(UK[,c("Month_Year","gvkey")])]

#Remove excess columns
UK$return <- NULL
UK$datadate <- NULL

# Transform returns into 0.2 = 20% notation as in original UK dataset
UK$ret <- UK$ret -1

# Rename columns to be equivalent to Original UK dataset
names(UK)[names(UK) == 'Month_Year'] <- 'date'
names(UK)[names(UK) == 'gvkey'] <- 'PERMNO'
names(UK)[names(UK) == 'ret'] <- 'RET'
names(UK)[names(UK) == 'conm'] <- 'COMNAM'
names(UK)[names(UK) == 'cshoc'] <- 'SHROUT'
names(UK)[names(UK) == 'prccd'] <- 'PRC'


#Rename inflation dataset accordingly

names(UK_Inflation)[names(UK_Inflation) == 'UK'] <- 'inflation'

# Add Inflation Data
UK$date <- as.numeric(as.character(UK$date))
Original1 <- join(UK,UK_Inflation,by=c("date"), type="inner")
...
UK_Shares_Outstanding <- select(read.csv("~/Desktop/MA/Data/full_shares_set_global.csv") , gvkey, datadate, cshoi)  # Retreived from quarterly fundamentals compUKtat
UK_Shares_Outstanding <- transform(UK_Shares_Outstanding, datadate = as.Date(as.character(datadate), "%Y%m%d"))
UK_Shares_Outstanding <- as.data.table(UK_Shares_Outstanding)
library(lubridate)
UK_Shares_Outstanding <- UK_Shares_Outstanding[rep(1:.N, each = 12),]
UK_Shares_Outstanding[, year := datadate]
UK_Shares_Outstanding[, datadate := {day(datadate) <- 1; datadate}]
UK_Shares_Outstanding[, datadate := {month(datadate) <- month(datadate) - 0:11; datadate}, by = year]
UK_Shares_Outstanding$datadate <- gsub("-", "", UK_Shares_Outstanding$datadate)
UK_Shares_Outstanding$year <- NULL
UK_Shares_Outstanding$cshoi <- UK_Shares_Outstanding$cshoi *1000000
UK_Shares_Outstanding$datadate <- substr(UK_Shares_Outstanding$datadate, 1,6)

names(UK_Shares_Outstanding)[names(UK_Shares_Outstanding) == 'gvkey'] <- 'PERMNO'
names(UK_Shares_Outstanding)[names(UK_Shares_Outstanding) == 'datadate'] <- 'date'

dummy <- InflaData[is.na(InflaData$mktcap),]
dummy <- join(UK_Shares_Outstanding, dummy, by=c("date","PERMNO"), type="right")
...
FF5 <- read_excel("~/Desktop/MA/Data/FF5_UK.xlsx")
...
UK_Inflation$date <- as.numeric(as.character(UK_Inflation$date))


# Add the inflation column
Rebalanced_Data <- merge(Rebalanced_Data,UK_Inflation,by=c("date"))
