...
#Import US Dataset
US_Data_Date <- read.csv("~/Desktop/MA/Data/US_Data.csv") 

# Include only stocks in domestic currency
US_Data_Date <- US_Data_Date[which(US_Data_Date$curcdd == "USD"),]

# Include only common stocks
US_Data_Date <- US_Data_Date[which(US_Data_Date$tpci == 0),]

US_Data_Date$curcdd <- NULL
US_Data_Date$tpci <- NULL

### Add Inflation column to dataset
US_Inflation <- select(read_excel("Desktop/MA/Data/Inflation-data.xlsx"), date, US)# Headline CPI; Data from Worldbank: https://www.worldbank.org/en/research/brief/inflation-database ; (Monthly inflation following this methodology: https://www.uvm.edu/~awoolf/classes/spring2005/ec11/calculating_inflation.html)
US_Inflation <-  US_Inflation %>%mutate(US=lag(US)) %>%na.omit()

# Remove ADR´s 
US_Data_Date$ADR <- substr(US_Data_Date$iid, 1,2)
US_Data_Date$ADR <- as.numeric(as.character(US_Data_Date$ADR))
US_Data_Date <- US_Data_Date[!US_Data_Date$ADR >= 90,]
US_Data_Date$ADR <- NULL 

#Create separate identification column
Identification <- US_Data_Date[c("gvkey","ggroup", "gind", "naics", "sic","conm")]
...
US_Data_Date$ggroup  <- NULL
US_Data_Date$gind  <- NULL
US_Data_Date$naics  <- NULL
US_Data_Date$sic  <- NULL

#Exclude rows which only have the dividend information of the secondary issue (div amount of primary issue is copied)

US_Data_Date <- US_Data_Date[!is.na(US_Data_Date$prccd),]

#create merged column to address the problem of multiple issues
US_Data_Date$merged <- paste(US_Data_Date$gvkey, US_Data_Date$iid)

# Replace missing trfd values with last available value
US_Data_Date <- as.data.frame(US_Data_Date %>% group_by(merged) %>% fill(trfd) %>% fill(trfd))
US_Data_Date$trfd[is.na(US_Data_Date$trfd)] <- 1

US_Data_Date$iid <- NULL
US_Data_Date$mktcap <-abs(US_Data_Date$prccd)*US_Data_Date$cshoc

dummy <- aggregate(mktcap ~ datadate + gvkey, US_Data_Date, na.action=na.pass, sum)  
names(US_Data_Date)[names(US_Data_Date) == 'mktcap'] <- 'indiv_merged_mktcap'

US_Data_Date <- join(dummy,US_Data_Date,by=c("datadate", "gvkey"), type="inner")

US_Data_Date$frct <- US_Data_Date$indiv_merged_mktcap / US_Data_Date$mktcap
dummy<- US_Data_Date %>% group_by(gvkey, datadate) %>% summarise(n = n())

US_Data_Date <- join(US_Data_Date,dummy,by=c("datadate", "gvkey"), type="inner")

dummy <- US_Data_Date[is.na(US_Data_Date$frct),]
dummy$frct[is.na(dummy$frct)] <- 1/dummy$n

US_Data_Date <- US_Data_Date[!is.na(US_Data_Date$frct),]
US_Data_Date <- rbind(US_Data_Date,dummy)

US_Data_Date$indiv_merged_mktcap <- NULL
US_Data_Date$n <- NULL

US_Data_Date$PrAj_1 <- US_Data_Date$prccd / US_Data_Date$ajexdi
US_Data_Date$PrAjTR_1 <- US_Data_Date$PrAj_1 * US_Data_Date$trfd

library(data.table)

US_Data_Date <- as.data.table(US_Data_Date)
setkey(US_Data_Date, gvkey, datadate)

#Subset the div column
dividend <- US_Data_Date[,c("datadate", "gvkey", "div", "prccd", "frct", "merged", "ajexdi")]

US_Data_Date <- US_Data_Date[!is.infinite(US_Data_Date$PrAjTR_1),]
US_Data_Date[, return := PrAjTR_1 / shift(PrAjTR_1, fill = NA), by = merged]
US_Data_Date$return[is.na(US_Data_Date$return)] <- 1

# Remove observations with daily returns >20
US_Data_Date <- US_Data_Date[which(US_Data_Date$return <= 20),]
US_Data_Date <- US_Data_Date[which(US_Data_Date$return >= 0.05),]

US_Data_Date$frct_return <- US_Data_Date$return * US_Data_Date$frct
US_Data_Date$return <- NULL
dummy <- aggregate(frct_return ~ datadate + gvkey, US_Data_Date, na.action=na.pass,sum)
names(dummy)[names(dummy) == 'frct_return'] <- 'return'

US_Data_Date <- join(US_Data_Date, dummy, by=c("datadate","gvkey"), type="inner")

US_Data_Date <- US_Data_Date[!duplicated(US_Data_Date[,c("datadate","gvkey")]),]

US_Data_Date$Month_Year <- substr(US_Data_Date$datadate, 1,6)

#subset volatility dataset
volatility <- select(US_Data_Date, datadate, gvkey, return)
names(volatility)[names(volatility) == 'return_dummy'] <- 'return'


US_Data_Date$merged <- NULL
US_Data_Date$frct <- NULL
US_Data_Date$frct_return <- NULL

US_Data_Date$PrAj_1  <- NULL
US_Data_Date$PrAjTR_1 <- NULL

US_Data_Date$ajexdi <- NULL
US_Data_Date$trfd <- NULL
US_Data_Date$div <- NULL
...
library(data.table)
dummy <- setDT(US_Data_Date)[,prod(return), by=.(Month_Year,gvkey)]

#merge the datasets 
US <- join(dummy, US_Data_Date, by=c("Month_Year","gvkey"), type="inner")
rm(dummy)

#rename monthly return column in ret
names(US)[names(US) == 'V1'] <- 'ret'

# Remove the duplicates
US <- US[!duplicated(US[,c("Month_Year","gvkey")])]

#Remove excess columns
US$return <- NULL
US$datadate <- NULL

# Transform returns into 0.2 = 20% notation as in original US dataset
US$ret <- US$ret -1

# Rename columns to be equivalent to Original US dataset
names(US)[names(US) == 'Month_Year'] <- 'date'
names(US)[names(US) == 'gvkey'] <- 'PERMNO'
names(US)[names(US) == 'ret'] <- 'RET'
names(US)[names(US) == 'conm'] <- 'COMNAM'
names(US)[names(US) == 'cshoc'] <- 'SHROUT'
names(US)[names(US) == 'prccd'] <- 'PRC'


#Rename inflation dataset accordingly

names(US_Inflation)[names(US_Inflation) == 'US'] <- 'inflation'

# Add Inflation Data
US$date <- as.numeric(as.character(US$date))
Original1 <- join(US,US_Inflation,by=c("date"), type="inner")
...
US_Shares_Outstanding <- select(read.csv("~/Desktop/MA/Data/full_shares_set.csv") , gvkey, datadate, cshoq)  # Retreived from quarterly fundamentals compustat
US_Shares_Outstanding <- transform(US_Shares_Outstanding, datadate = as.Date(as.character(datadate), "%Y%m%d"))
US_Shares_Outstanding <- as.data.table(US_Shares_Outstanding)
library(lubridate)
US_Shares_Outstanding <- US_Shares_Outstanding[rep(1:.N, each = 3),]
US_Shares_Outstanding[, quarter := datadate]
US_Shares_Outstanding[, datadate := {day(datadate) <- 1; datadate}]
US_Shares_Outstanding[, datadate := {month(datadate) <- month(datadate) - 1:(-1); datadate}, by = quarter]
US_Shares_Outstanding[, datadate := datadate -1]
US_Shares_Outstanding[.(seq(as.IDate("1985-01-31	"), as.IDate("2021-10-31"), by="month") - 1L), 
                      on=.(datadate), roll=-Inf]
US_Shares_Outstanding$datadate <- gsub("-", "", US_Shares_Outstanding$datadate)
US_Shares_Outstanding$quarter <- NULL
US_Shares_Outstanding$cshoq <- US_Shares_Outstanding$cshoq *1000000
US_Shares_Outstanding$datadate <- substr(US_Shares_Outstanding$datadate, 1,6)

names(US_Shares_Outstanding)[names(US_Shares_Outstanding) == 'gvkey'] <- 'PERMNO'
names(US_Shares_Outstanding)[names(US_Shares_Outstanding) == 'datadate'] <- 'date'

dummy <- InflaData[is.na(InflaData$mktcap),]
dummy <- join(US_Shares_Outstanding, dummy, by=c("date","PERMNO"), type="right")
...
rm(dummy)
rm(US_Shares_Outstanding)
InflaData <- InflaData[which(InflaData$date >= 198512),]
...
FF5 <- read_excel("~/Desktop/MA/Data/FF5_US.xlsx")
...
US_Inflation$date <- as.numeric(as.character(US_Inflation$date))


# Add the inflation column
Rebalanced_Data <- merge(Rebalanced_Data,US_Inflation,by=c("date"))
