...
#Import Italy Dataset
Italy_Data_Date <- read.csv("~/Desktop/MA/Data/Italy_Data.csv") 

Italy_Data_Date$fic <- NULL

# Include only common stocks
Italy_Data_Date <- Italy_Data_Date[which(Italy_Data_Date$tpci == 0),]

# Convert DM to EUR
EUR_DM_Conversion <- Italy_Data_Date[which(Italy_Data_Date$curcdd == "ITL"),]
Italy_Data_Date <- Italy_Data_Date[which(Italy_Data_Date$curcdd == "EUR"),]

EUR_DM_Conversion$prccd <- EUR_DM_Conversion$prccd / 1936.27
EUR_DM_Conversion$div <- EUR_DM_Conversion$div / 1936.27

Italy_Data_Date <- rbind(Italy_Data_Date, EUR_DM_Conversion)

rm(EUR_DM_Conversion)

Italy_Data_Date$curcdd <- NULL
Italy_Data_Date$tpci <- NULL

### Add Inflation column to dataset
Italy_Inflation <- select(read_excel("~/Desktop/MA/Data/Inflation-data.xlsx"), date, Italy)# Headline CPI; Data from Worldbank: https://www.worldbank.org/en/research/brief/inflation-database ; (Monthly inflation following this methodology: https://www.uvm.edu/~awoolf/classes/spring2005/ec11/calculating_inflation.html)
Italy_Inflation <-  Italy_Inflation %>%mutate(Italy=lag(Italy)) %>%na.omit()

# Remove ADR´s 
Italy_Data_Date$ADR <- substr(Italy_Data_Date$iid, 1,2)
Italy_Data_Date$ADR <- as.numeric(as.character(Italy_Data_Date$ADR))
Italy_Data_Date <- Italy_Data_Date[!Italy_Data_Date$ADR >= 90,]
Italy_Data_Date$ADR <- NULL 

#Create separate identification column
Identification <- Italy_Data_Date[c("gvkey","ggroup", "gind", "naics", "sic","conm")]
...
Italy_Data_Date$ggroup  <- NULL
Italy_Data_Date$gind  <- NULL
Italy_Data_Date$naics  <- NULL
Italy_Data_Date$sic  <- NULL

#Exclude rows which only have the dividend information of the secondary issue (div amount of primary issue is copied)

Italy_Data_Date <- Italy_Data_Date[!is.na(Italy_Data_Date$prccd),]

#create merged column to address the problem of multiple issues
Italy_Data_Date$merged <- paste(Italy_Data_Date$gvkey, Italy_Data_Date$iid)

# Replace missing trfd values with last available value
Italy_Data_Date <- as.data.frame(Italy_Data_Date %>% group_by(merged) %>% fill(trfd) %>% fill(trfd))
Italy_Data_Date$trfd[is.na(Italy_Data_Date$trfd)] <- 1

Italy_Data_Date$iid <- NULL
Italy_Data_Date$mktcap <-abs(Italy_Data_Date$prccd)*Italy_Data_Date$cshoc

dummy <- aggregate(mktcap ~ datadate + gvkey, Italy_Data_Date, na.action=na.pass, sum)  
names(Italy_Data_Date)[names(Italy_Data_Date) == 'mktcap'] <- 'indiv_merged_mktcap'

Italy_Data_Date <- join(dummy,Italy_Data_Date,by=c("datadate", "gvkey"), type="inner")

Italy_Data_Date$frct <- Italy_Data_Date$indiv_merged_mktcap / Italy_Data_Date$mktcap
dummy<- Italy_Data_Date %>% group_by(gvkey, datadate) %>% summarise(n = n())

Italy_Data_Date <- join(Italy_Data_Date,dummy,by=c("datadate", "gvkey"), type="inner")

dummy <- Italy_Data_Date[is.na(Italy_Data_Date$frct),]
dummy$frct[is.na(dummy$frct)] <- 1/dummy$n

Italy_Data_Date <- Italy_Data_Date[!is.na(Italy_Data_Date$frct),]
Italy_Data_Date <- rbind(Italy_Data_Date,dummy)

Italy_Data_Date$indiv_merged_mktcap <- NULL
Italy_Data_Date$n <- NULL

Italy_Data_Date$PrAj_1 <- Italy_Data_Date$prccd / Italy_Data_Date$ajexdi
Italy_Data_Date$PrAjTR_1 <- Italy_Data_Date$PrAj_1 * Italy_Data_Date$trfd

library(data.table)

Italy_Data_Date <- as.data.table(Italy_Data_Date)
setkey(Italy_Data_Date, gvkey, datadate)

#Subset the div column
dividend <- Italy_Data_Date[,c("datadate", "gvkey", "div", "prccd", "frct", "merged", "ajexdi")]

Italy_Data_Date <- Italy_Data_Date[!is.infinite(Italy_Data_Date$PrAjTR_1),]
Italy_Data_Date[, return := PrAjTR_1 / shift(PrAjTR_1, fill = NA), by = merged]
Italy_Data_Date$return[is.na(Italy_Data_Date$return)] <- 1

# Remove observations with daily returns >20
Italy_Data_Date <- Italy_Data_Date[which(Italy_Data_Date$return <= 20),]
Italy_Data_Date <- Italy_Data_Date[which(Italy_Data_Date$return >= 0.05),]

Italy_Data_Date$frct_return <- Italy_Data_Date$return * Italy_Data_Date$frct
Italy_Data_Date$return <- NULL
dummy <- aggregate(frct_return ~ datadate + gvkey, Italy_Data_Date, na.action=na.pass,sum)
names(dummy)[names(dummy) == 'frct_return'] <- 'return'

Italy_Data_Date <- join(Italy_Data_Date, dummy, by=c("datadate","gvkey"), type="inner")

Italy_Data_Date <- Italy_Data_Date[!duplicated(Italy_Data_Date[,c("datadate","gvkey")]),]

Italy_Data_Date$Month_Year <- substr(Italy_Data_Date$datadate, 1,6)

#subset volatility dataset
volatility <- select(Italy_Data_Date, datadate, gvkey, return)
names(volatility)[names(volatility) == 'return_dummy'] <- 'return'


Italy_Data_Date$merged <- NULL
Italy_Data_Date$frct <- NULL
Italy_Data_Date$frct_return <- NULL

Italy_Data_Date$PrAj_1  <- NULL
Italy_Data_Date$PrAjTR_1 <- NULL

Italy_Data_Date$ajexdi <- NULL
Italy_Data_Date$trfd <- NULL
Italy_Data_Date$div <- NULL
...
library(data.table)
dummy <- setDT(Italy_Data_Date)[,prod(return), by=.(Month_Year,gvkey)]

#merge the datasets 
Italy <- join(dummy, Italy_Data_Date, by=c("Month_Year","gvkey"), type="inner")
rm(dummy)

#rename monthly return column in ret
names(Italy)[names(Italy) == 'V1'] <- 'ret'

# Remove the duplicates
Italy <- Italy[!duplicated(Italy[,c("Month_Year","gvkey")])]

#Remove excess columns
Italy$return <- NULL
Italy$datadate <- NULL

# Transform returns into 0.2 = 20% notation as in original Italy dataset
Italy$ret <- Italy$ret -1

# Rename columns to be equivalent to Original Italy dataset
names(Italy)[names(Italy) == 'Month_Year'] <- 'date'
names(Italy)[names(Italy) == 'gvkey'] <- 'PERMNO'
names(Italy)[names(Italy) == 'ret'] <- 'RET'
names(Italy)[names(Italy) == 'conm'] <- 'COMNAM'
names(Italy)[names(Italy) == 'cshoc'] <- 'SHROUT'
names(Italy)[names(Italy) == 'prccd'] <- 'PRC'


#Rename inflation dataset accordingly

names(Italy_Inflation)[names(Italy_Inflation) == 'Italy'] <- 'inflation'

# Add Inflation Data
Italy$date <- as.numeric(as.character(Italy$date))
Original1 <- join(Italy,Italy_Inflation,by=c("date"), type="inner")
...
rm(Peter, Peter1, Original1, Italy)
...
Italy_Shares_Outstanding <- select(read.csv("~/Desktop/MA/Data/full_shares_set_global.csv") , gvkey, datadate, cshoi)  # Retreived from quarterly fundamentals compItalytat
Italy_Shares_Outstanding <- transform(Italy_Shares_Outstanding, datadate = as.Date(as.character(datadate), "%Y%m%d"))
Italy_Shares_Outstanding <- as.data.table(Italy_Shares_Outstanding)
library(lubridate)
Italy_Shares_Outstanding <- Italy_Shares_Outstanding[rep(1:.N, each = 12),]
Italy_Shares_Outstanding[, year := datadate]
Italy_Shares_Outstanding[, datadate := {day(datadate) <- 1; datadate}]
Italy_Shares_Outstanding[, datadate := {month(datadate) <- month(datadate) - 0:11; datadate}, by = year]
Italy_Shares_Outstanding$datadate <- gsub("-", "", Italy_Shares_Outstanding$datadate)
Italy_Shares_Outstanding$year <- NULL
Italy_Shares_Outstanding$cshoi <- Italy_Shares_Outstanding$cshoi *1000000
Italy_Shares_Outstanding$datadate <- substr(Italy_Shares_Outstanding$datadate, 1,6)

names(Italy_Shares_Outstanding)[names(Italy_Shares_Outstanding) == 'gvkey'] <- 'PERMNO'
names(Italy_Shares_Outstanding)[names(Italy_Shares_Outstanding) == 'datadate'] <- 'date'

dummy <- InflaData[is.na(InflaData$mktcap),]
dummy <- join(Italy_Shares_Outstanding, dummy, by=c("date","PERMNO"), type="right")
...
FF5 <- read_excel("~/Desktop/MA/Data/FF5_Italy.xlsx")
...
Italy_Inflation$date <- as.numeric(as.character(Italy_Inflation$date))


# Add the inflation column
Rebalanced_Data <- merge(Rebalanced_Data,Italy_Inflation,by=c("date"))
