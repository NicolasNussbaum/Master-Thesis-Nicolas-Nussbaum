...
#Import Switzerland Dataset
Switzerland_Data_Date <- read.csv("~/Desktop/MA/Data/Switzerland_Data.csv") 

Switzerland_Data_Date$fic <- NULL

# Include only stocks in domestic currency
Switzerland_Data_Date <- Switzerland_Data_Date[which(Switzerland_Data_Date$curcdd == "CHF"),]

# Include only common stocks
Switzerland_Data_Date <- Switzerland_Data_Date[which(Switzerland_Data_Date$tpci == 0),]

Switzerland_Data_Date$curcdd <- NULL
Switzerland_Data_Date$tpci <- NULL

### Add Inflation column to dataset
Switzerland_Inflation <- select(read_excel("Desktop/MA/Data/Inflation-data.xlsx"), date, Switzerland)# Headline CPI; Data from Worldbank: https://www.worldbank.org/en/research/brief/inflation-database ; (Monthly inflation following this methodology: https://www.uvm.edu/~awoolf/classes/spring2005/ec11/calculating_inflation.html)
Switzerland_Inflation <-  Switzerland_Inflation %>%mutate(Switzerland=lag(Switzerland)) %>%na.omit()

# Remove ADR´s 
Switzerland_Data_Date$ADR <- substr(Switzerland_Data_Date$iid, 1,2)
Switzerland_Data_Date$ADR <- as.numeric(as.character(Switzerland_Data_Date$ADR))
Switzerland_Data_Date <- Switzerland_Data_Date[!Switzerland_Data_Date$ADR >= 90,]
Switzerland_Data_Date$ADR <- NULL 

#Create separate identification column
Identification <- Switzerland_Data_Date[c("gvkey","ggroup", "gind", "naics", "sic", "conm")]
...
Switzerland_Data_Date$ggroup  <- NULL
Switzerland_Data_Date$gind  <- NULL
Switzerland_Data_Date$naics  <- NULL
Switzerland_Data_Date$sic  <- NULL

#Exclude rows which only have the dividend information of the secondary issue (div amount of primary issue is copied)

Switzerland_Data_Date <- Switzerland_Data_Date[!is.na(Switzerland_Data_Date$prccd),]

#create merged column to address the problem of multiple issues
Switzerland_Data_Date$merged <- paste(Switzerland_Data_Date$gvkey, Switzerland_Data_Date$iid)

# Replace missing trfd values with last available value
Switzerland_Data_Date <- as.data.frame(Switzerland_Data_Date %>% group_by(merged) %>% fill(trfd) %>% fill(trfd))
Switzerland_Data_Date$trfd[is.na(Switzerland_Data_Date$trfd)] <- 1

Switzerland_Data_Date$iid <- NULL
Switzerland_Data_Date$mktcap <-abs(Switzerland_Data_Date$prccd)*Switzerland_Data_Date$cshoc

dummy <- aggregate(mktcap ~ datadate + gvkey, Switzerland_Data_Date, na.action=na.pass, sum)  
names(Switzerland_Data_Date)[names(Switzerland_Data_Date) == 'mktcap'] <- 'indiv_merged_mktcap'

Switzerland_Data_Date <- join(dummy,Switzerland_Data_Date,by=c("datadate", "gvkey"), type="inner")

Switzerland_Data_Date$frct <- Switzerland_Data_Date$indiv_merged_mktcap / Switzerland_Data_Date$mktcap
dummy<- Switzerland_Data_Date %>% group_by(gvkey, datadate) %>% summarise(n = n())

Switzerland_Data_Date <- join(Switzerland_Data_Date,dummy,by=c("datadate", "gvkey"), type="inner")

dummy <- Switzerland_Data_Date[is.na(Switzerland_Data_Date$frct),]
dummy$frct[is.na(dummy$frct)] <- 1/dummy$n

Switzerland_Data_Date <- Switzerland_Data_Date[!is.na(Switzerland_Data_Date$frct),]
Switzerland_Data_Date <- rbind(Switzerland_Data_Date,dummy)

Switzerland_Data_Date$indiv_merged_mktcap <- NULL
Switzerland_Data_Date$n <- NULL

Switzerland_Data_Date$PrAj_1 <- Switzerland_Data_Date$prccd / Switzerland_Data_Date$ajexdi
Switzerland_Data_Date$PrAjTR_1 <- Switzerland_Data_Date$PrAj_1 * Switzerland_Data_Date$trfd

library(data.table)

Switzerland_Data_Date <- as.data.table(Switzerland_Data_Date)
setkey(Switzerland_Data_Date, gvkey, datadate)

#Subset the div column
dividend <- Switzerland_Data_Date[,c("datadate", "gvkey", "div", "prccd", "frct", "merged", "ajexdi")]

Switzerland_Data_Date <- Switzerland_Data_Date[!is.infinite(Switzerland_Data_Date$PrAjTR_1),]
Switzerland_Data_Date[, return := PrAjTR_1 / shift(PrAjTR_1, fill = NA), by = merged]
Switzerland_Data_Date$return[is.na(Switzerland_Data_Date$return)] <- 1

# Remove observations with daily returns >20
Switzerland_Data_Date <- Switzerland_Data_Date[which(Switzerland_Data_Date$return <= 20),]
Switzerland_Data_Date <- Switzerland_Data_Date[which(Switzerland_Data_Date$return >= 0.05),]

Switzerland_Data_Date$frct_return <- Switzerland_Data_Date$return * Switzerland_Data_Date$frct
Switzerland_Data_Date$return <- NULL
dummy <- aggregate(frct_return ~ datadate + gvkey, Switzerland_Data_Date, na.action=na.pass,sum)
names(dummy)[names(dummy) == 'frct_return'] <- 'return'

Switzerland_Data_Date <- join(Switzerland_Data_Date, dummy, by=c("datadate","gvkey"), type="inner")

Switzerland_Data_Date <- Switzerland_Data_Date[!duplicated(Switzerland_Data_Date[,c("datadate","gvkey")]),]

Switzerland_Data_Date$Month_Year <- substr(Switzerland_Data_Date$datadate, 1,6)

#subset volatility dataset
volatility <- select(Switzerland_Data_Date, datadate, gvkey, return)
names(volatility)[names(volatility) == 'return_dummy'] <- 'return'


Switzerland_Data_Date$merged <- NULL
Switzerland_Data_Date$frct <- NULL
Switzerland_Data_Date$frct_return <- NULL

Switzerland_Data_Date$PrAj_1  <- NULL
Switzerland_Data_Date$PrAjTR_1 <- NULL

Switzerland_Data_Date$ajexdi <- NULL
Switzerland_Data_Date$trfd <- NULL
Switzerland_Data_Date$div <- NULL
...
library(data.table)
dummy <- setDT(Switzerland_Data_Date)[,prod(return), by=.(Month_Year,gvkey)]

#merge the datasets 
Switzerland <- join(dummy, Switzerland_Data_Date, by=c("Month_Year","gvkey"), type="inner")
rm(dummy)

#rename monthly return column in ret
names(Switzerland)[names(Switzerland) == 'V1'] <- 'ret'

# Remove the duplicates
Switzerland <- Switzerland[!duplicated(Switzerland[,c("Month_Year","gvkey")])]

#Remove excess columns
Switzerland$return <- NULL
Switzerland$datadate <- NULL

# Transform returns into 0.2 = 20% notation as in original Switzerland dataset
Switzerland$ret <- Switzerland$ret -1

# Rename columns to be equivalent to Original Switzerland dataset
names(Switzerland)[names(Switzerland) == 'Month_Year'] <- 'date'
names(Switzerland)[names(Switzerland) == 'gvkey'] <- 'PERMNO'
names(Switzerland)[names(Switzerland) == 'ret'] <- 'RET'
names(Switzerland)[names(Switzerland) == 'conm'] <- 'COMNAM'
names(Switzerland)[names(Switzerland) == 'cshoc'] <- 'SHROUT'
names(Switzerland)[names(Switzerland) == 'prccd'] <- 'PRC'


#Rename inflation dataset accordingly

names(Switzerland_Inflation)[names(Switzerland_Inflation) == 'Switzerland'] <- 'inflation'

# Add Inflation Data
Switzerland$date <- as.numeric(as.character(Switzerland$date))
Original1 <- join(Switzerland,Switzerland_Inflation,by=c("date"), type="inner")
...
rm(Peter, Peter1, Original1, Switzerland)
...
Switzerland_Shares_Outstanding <- select(read.csv("~/Desktop/MA/Data/full_shares_set_global.csv") , gvkey, datadate, cshoi)  # Retreived from quarterly fundamentals compSwitzerlandtat
Switzerland_Shares_Outstanding <- transform(Switzerland_Shares_Outstanding, datadate = as.Date(as.character(datadate), "%Y%m%d"))
Switzerland_Shares_Outstanding <- as.data.table(Switzerland_Shares_Outstanding)
library(lubridate)
Switzerland_Shares_Outstanding <- Switzerland_Shares_Outstanding[rep(1:.N, each = 12),]
Switzerland_Shares_Outstanding[, year := datadate]
Switzerland_Shares_Outstanding[, datadate := {day(datadate) <- 1; datadate}]
Switzerland_Shares_Outstanding[, datadate := {month(datadate) <- month(datadate) - 0:11; datadate}, by = year]
Switzerland_Shares_Outstanding$datadate <- gsub("-", "", Switzerland_Shares_Outstanding$datadate)
Switzerland_Shares_Outstanding$year <- NULL
Switzerland_Shares_Outstanding$cshoi <- Switzerland_Shares_Outstanding$cshoi *1000000
Switzerland_Shares_Outstanding$datadate <- substr(Switzerland_Shares_Outstanding$datadate, 1,6)

names(Switzerland_Shares_Outstanding)[names(Switzerland_Shares_Outstanding) == 'gvkey'] <- 'PERMNO'
names(Switzerland_Shares_Outstanding)[names(Switzerland_Shares_Outstanding) == 'datadate'] <- 'date'

dummy <- InflaData[is.na(InflaData$mktcap),]
dummy <- join(Switzerland_Shares_Outstanding, dummy, by=c("date","PERMNO"), type="right")
...
rm(dummy)
rm(Switzerland_Shares_Outstanding)
...
FF5 <- read_excel("~/Desktop/MA/Data/FF5_Switzerland.xlsx")
...
Switzerland_Inflation$date <- as.numeric(as.character(Switzerland_Inflation$date))


# Add the inflation column
Rebalanced_Data <- merge(Rebalanced_Data,Switzerland_Inflation,by=c("date"))
